...
<div class="pt-m">
    ...
    <div>
        <div class="form-group collapsible mb-none" component="collapsible" id="discord-field">
            <button refs="collapsible@trigger" type="button" class="collapse-title text-link" aria-expanded="false">
                <label for="discord-id">{{ trans('settings.users_discord_id') }}</label>
            </button>
            <div refs="collapsible@content" class="collapse-content stretch-inputs">
                <p class="small">{{ trans('settings.users_discord_id_desc') }}</p>
                @include('form.text', ['name' => 'discord_id'])
            </div>
        </div>
    </div>
</div>
